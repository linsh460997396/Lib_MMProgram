...
namespace MetalMaxSystem
{
    ...
    public class KeyboardHook
    {
        ...
        /// <param name="hInstance">包含lpfn所指的子程的DLL的句柄.若dwThreadId参数为0或是一由别的进程创建的线程的标识,hMod必须为包含lpfn所指的子程的DLL的句柄.
        /// 若dwThreadId标识当前进程创建的一个线程,而且子程代码位于当前进程,hMod必须为NULL.
        /// 可以很简单的设定其为本应用程序的实例句柄(this.Handle).</param>
        /// <param name="threadId">与安装的钩子子程相关联的线程的标识符.
        /// 若为0,钩子子程与所有的线程关联,即为全局钩子.
        /// </param>
        /// <returns>若函数成功,返回值是钩子的句柄.若函数失败,返回值为0.
        ...
        /// <returns>若函数成功,返回值为true.若函数失败,返回值为false.</returns>
        [DllImport("user32.dll", CharSet = CharSet.Auto, CallingConvention = CallingConvention.StdCall)]
        ...
        /// <param name="wParam">与钩子事件相关的wParam参数,其含义取决于具体的钩子类型.</param>
        /// <param name="lParam">与钩子事件相关的lParam参数,通常是一个指向事件相关结构的指针.</param>
        /// <returns>若函数成功,返回值是由下一个钩子过程返回的值.若当前钩子过程是钩子链中的最后一个,或者钩子链被某个钩子过程中断,则返回特定的值.</returns>
        [DllImport("user32.dll", CharSet = CharSet.Auto, CallingConvention = CallingConvention.StdCall)]
        ...
        /// </summary>
        /// <param name="lpModuleName">要获取句柄的模块的名称.若为NULL,则获取当前进程的模块句柄.</param>
        /// <returns>若函数成功,返回值是指定模块的句柄.若函数失败,返回值为NULL.</returns>
        ...
        /// 键盘的系统回调函数
        /// </summary>
        /// <param name="nCode">若代码小于零,则挂钩过程必须将消息传递给CallNextHookEx函数,而无需进一步处理,并且应返回CallNextHookEx返回的值.此参数可以是下列值之一(见虚拟键码)</param>
        ...
        {
            //若该消息被丢弃(nCode<0)或者没有事件绑定处理程序则不会触发事件
...
//‌不保证原子性‌:重要的是要理解volatile并不保证对字段的访问是原子的.也就是说,若多个线程同时对同一个volatile字段进行读写操作,仍然可能会出现竞态条件和数据不一致性问题.因此,volatile通常用于简单的状态标志或检查点,而不是用于需要原子性操作的场景.
...
//特定情况下,若打算让外部代码能够附加和分离事件处理程序(例如当键盘事件发生时),那么应该用event关键字,这样就可利用C#事件模型来管理事件处理程序,并确保代码具有良好的封装性和可维护性.
