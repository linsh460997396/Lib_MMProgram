...
namespace MetalMaxSystem
{
    ...
    public static class SubUpdate
    {
        ...
        /// <summary>
        /// 副循环触发器Update阶段,用来实现周期循环的计时器(支持小数毫秒间隔)
        /// </summary>
        ...
        /// <summary>
        /// 副循环触发器Update事件运行次数(使用ulong类型防止溢出)
        /// </summary>
        ...
        /// <summary>
        /// 副循环触发器Update阶段间隔运行时间(毫秒，支持小数如62.5ms)
        /// </summary>
        ...
        /// <summary>
        /// 重置副循环触发器状态,允许重新启动(线程终止后调用此方法可恢复初始状态)
        /// </summary>
        ...
        /// <summary>
        /// 副循环触发器主体事件发布动作(重复执行则什么也不做),若Update阶段属性未定义则默认每轮前摇0ms、间隔1000ms
        /// </summary>
...
//// 方式一：单独注册单个函数
//// 例如：MMCore.RegistEntryEventFuncref(Entry.SubAwake, OnSubAwake);  // 注册副循环唤醒事件
////       MMCore.RegistEntryEventFuncref(Entry.SubStart, OnSubStart);  // 注册副循环开始事件
////       MMCore.RegistEntryEventFuncref(Entry.SubUpdate, OnSubUpdate);  // 注册副循环更新事件
////       MMCore.RegistEntryEventFuncref(Entry.SubEnd, OnSubEnd);  // 注册副循环结束事件
////       MMCore.RegistEntryEventFuncref(Entry.SubDestroy, OnSubDestroy);  // 注册副循环销毁事件
////
//// 方式二：使用多播委托注册多个函数
//// EntryEventFuncref subUpdateHandlers = null;
//// subUpdateHandlers += OnUpdate1;
//// subUpdateHandlers += OnUpdate2;
//// subUpdateHandlers += OnUpdate3;
//// MMCore.RegistEntryEventFuncref(Entry.SubUpdate, subUpdateHandlers);
////
//// 方式三：简洁事件注册（推荐，最简洁的方式）
//// SubUpdate.Awake += OnSubAwake;    // 注册副循环唤醒事件
//// SubUpdate.Start += OnSubStart;    // 注册副循环开始事件
//// SubUpdate.Update += OnSubUpdate;  // 注册副循环更新事件
//// SubUpdate.End += OnSubEnd;        // 注册副循环结束事件
//// SubUpdate.Destroy += OnSubDestroy; // 注册副循环销毁事件
////
//// 注销事件
//// SubUpdate.Awake -= OnSubAwake;    // 注销副循环唤醒事件
////
//// ⚠️ 注意：与 TimerUpdate/Trigger 的区别
//// - SubUpdate/MainUpdate 是静态类，**现在也支持多次 += 注册多个事件处理器**（使用 EventHandlerList）
//// - TimerUpdate/Trigger 是实例类，支持直接多次 += 注册多个事件处理器
//// - 两种方式可以混用：通过 += 注册的处理器 和 通过 MMCore.RegistEntryEventFuncref 注册的处理器都会被执行

//// 2. 配置参数（可选，不配置则使用默认值）
//SubUpdate.Duetime = 0;    // 前摇时间（毫秒）
//SubUpdate.Period = 1000;  // 更新间隔（毫秒，默认1000ms）

//// 3. 启动（创建独立线程运行）
//SubUpdate.Run(isBackground: true);

//// 4. 运行中可以读取状态
//ulong count = SubUpdate.InvokeCount;  // 获取Update执行次数

//// 5. 停止（设置标志，等待当前周期结束后优雅退出）
//SubUpdate.TimerStop = true;

//// 6. 停止后可以重新启动
//SubUpdate.Run(isBackground: true);  // OnDestroy已自动重置状态
